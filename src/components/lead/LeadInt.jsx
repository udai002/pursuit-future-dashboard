...
//           `http://localhost:3000/leadgen/leadgen/employee/${userDetails._id}?page=${page}`
//         );
...
//         `http://localhost:3000/leadgen/leadgen/${id}`,
//         {
//           method: 'PUT',
//           headers: {
...
const LeadInt = () => {
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        const res = await fetch(
          `http://localhost:3000/leadgen/leadgen/employee/${userDetails._id}?page=${page}&limit=${limit}`
        );
        if (!res.ok) throw new Error('Failed to fetch leads');
